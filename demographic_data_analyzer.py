...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = round(100 * (df[mask].shape[0] / df.shape[0]), 1)
    ...
    mask = df['salary'] == '>50K'
    highest_earning_country = (df[mask]['native-country'].value_counts() / df['native-country'].value_counts()).dropna().sort_values().index[-1]
    highest_earning_country_percentage =  round(100 * (df[(df['native-country'] == highest_earning_country) & (mask)].shape[0] / df[df['native-country'] == highest_earning_country].shape[0]), 1)

    # Identify the most popular occupation for those who earn >50K in India.
    mask = (df['native-country'] == 'India') & (df['salary'] == '>50K')
    top_IN_occupation = df[mask].occupation.value_counts().sort_values().index[-1]
